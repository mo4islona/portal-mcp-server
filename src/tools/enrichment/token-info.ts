...
SUPPORTED CHAINS: ethereum, base, arbitrum, optimism, polygon, avalanche, bsc`,
    {
      ...
      address: z
        .string()
        .optional()
        .describe("Token contract address (if looking up specific token)"),
      symbol: z
        .string()
        .optional()
        .describe("Token symbol (e.g., 'USDC', 'WETH')"),
      limit: z
        .number()
        .optional()
        .default(100)
        .describe("Max tokens to return when fetching all (default 100)"),
    },
    async ({ chain, address, symbol, limit }) => {
      ...
      if (address) {
        ...
        return formatResult(
          token,
          `Found token: ${token.name} (${token.symbol})`,
          {
            metadata: {
        ...
        return formatResult(
          tokens,
          `Found ${tokens.length} token(s) with symbol ${symbol}`,
          {
            metadata: {
              query_start_time: queryStartTime,
            },
