  ...
  server.tool(
    ...
    "Analyze smart contract activity. Returns interaction count, unique callers, events emitted, and top users. Perfect for monitoring contract usage, identifying power users, or detecting unusual activity patterns.",
    {
      ...
      timeframe: z
        ...
        .default("1000")
        .describe(
          "Analysis period: '1h'=~1800 blocks, '24h'=~43200, '7d'=~302400, or block count",
        ),
      include_events: z
        .boolean()
        .optional()
        .default(true)
        .describe("Include event log summary"),
    },
    async ({ dataset, contract_address, timeframe, include_events }) => {
      ...
      if (chainType !== "evm") {
        throw new Error(
          "portal_get_contract_activity is only for EVM chains",
        );
      }
      ...
      if (include_events) {
        ...
        const rawEvents = eventResults.flatMap(
