...
export async function getCoinGeckoTokenList(chain: string): Promise<CoinGeckoToken[]> {
  ...
  if (!url) {
    throw new Error(`No CoinGecko token list available for chain: ${chain}. Available: ${Object.keys(COINGECKO_TOKEN_LISTS).join(", ")}`);
  }
...
export async function getChainTvl(chain: string): Promise<{ tvl: number; protocols: number }> {
  return withCache(`defillama:chain:${chain}`, CACHE_TTL, async () => {
    ...
    const chainData = chains.find((c) =>
      c.name?.toLowerCase() === normalizedChain ||
      c.gecko_id?.toLowerCase() === normalizedChain
    );
